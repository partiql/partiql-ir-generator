...
class TypeDomainSemanticCheckerTests {
    ...
    companion object {
        ...
        fun parametersForMiscErrorsTest() = listOf(
            ...
            TestCase(
                "(define some_domain (domain (product some_product x::(* int 1) y::(* int 1))))",
                makeErr(1, 64, SemanticErrorContext.MoreThanOneVariadicElement)
            ),

            TestCase(
                "(define some_domain (domain (product some_product x::(? ion))))",
                makeErr(1, 51, SemanticErrorContext.OptionalIonTypeElement)
            ),

            TestCase(
                "(define foo (domain)) (transform bar foo)",
                makeErr(1, 23, SemanticErrorContext.SourceDomainDoesNotExist("bar"))
            ),

            TestCase(
                "(define foo (domain)) (transform foo bar)",
        ...
        fun parametersForPerumteErrorsTest() = listOf(
            TestCase(
                "(define foo (permute_domain nonexistent))",
                makeErr(1, 13, SemanticErrorContext.DomainPermutesNonExistentDomain("foo", "nonexistent"))
            ),

            TestCase(
                "(define some_domain (domain)) (define some_permuted_domain (permute_domain some_domain (exclude nonexistent)))",
                makeErr(1, 60, SemanticErrorContext.CannotRemoveNonExistentType("nonexistent", "some_permuted_domain", "some_domain"))
            ),

            TestCase(
                "(define some_domain (domain)) (define some_permuted_domain (permute_domain some_domain (with nonexistent)))",
                makeErr(1, 88, SemanticErrorContext.CannotPermuteNonExistentSum("nonexistent", "some_permuted_domain", "some_domain"))
            ),

            TestCase(
                "(define some_domain (domain (sum some_sum (a x::int)))) (define some_permuted_domain (permute_domain some_domain (with some_sum (exclude nonexistent))))",
                makeErr(1, 114, SemanticErrorContext.CannotRemoveNonExistentSumVariant(sumTypeName = "some_sum", variantName = "nonexistent"))
            ),

            TestCase(
                "(define some_domain (domain)) (define some_domain (permute_domain some_domain (with int)))",
                makeErr(1, 79, SemanticErrorContext.CannotPermuteNonSumType("int"))
            ),

            TestCase(
                "(define some_domain (domain (product x::foo y::int))) (define some_domain (permute_domain some_domain (with foo)))",
                makeErr(1, 103, SemanticErrorContext.CannotPermuteNonSumType("foo"))
            ),

            TestCase(
                "(define some_domain (domain)) (define permuted_domain (permute_domain some_domain (exclude int)))",
                makeErr(1, 55, SemanticErrorContext.CannotRemoveBuiltinType("int"))
            ),

            TestCase(
                "(define some_domain (domain (product some_product x::undefined)))",
            ...
            TestCase(
                "(define some_domain (domain (sum non_empty_sum (a x::int)))) (define another_domain (permute_domain some_domain (with non_empty_sum (exclude a))))",
        ...
        fun parametersForNameErrorsTest2() = listOf(
            ...
            TestCase(
                "(define some_domain (domain (product some_product) (sum some_sum (some_product))))",
            ...
            TestCase(
                "(define some_domain (domain (sum some_sum (a_variant (some_field int) (some_field int)))))",
            ...
            TestCase(
                "(define some_domain (domain (record some_record (some_field int) (some_field int))))",
                makeErr(1, 66, SemanticErrorContext.DuplicateRecordElementTag("some_field"))
            ),

            // Duplicate product element tag
            TestCase(
                "(define some_domain (domain (product some_product some_field::int some_field::int)))",
