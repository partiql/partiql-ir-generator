...
/*
- Every domain will get one `VisitorBase` class.
- On that `VisitorBase` class, for every data type in the domain, there will be at least one method:
    - All types will get  a `visit_$dataType` function.
        - For sum types, it will be a dispatcher function that will dispatch to a function specific to the
        variant named: `visit_$dataType_$variant`
        - For product and record types, it will invoke element specific functions named:
        `visit_$dataType_$elementName`.

This is a lot of visit methods, however the extra methods allow inheritors to be a *lot* less concerned about *how*
the child nodes should be walked, and (except in the case where traversal order needs to be customized) will also allow
them to not have to be concerned with how to construct new nodes.

An assumption made by this approach is that the input domain is the same as the output domain.
 */
